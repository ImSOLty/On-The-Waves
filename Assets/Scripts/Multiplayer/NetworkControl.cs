...
public class NetworkControl : NetworkBehaviour
{
    ...
    private CanvasLookAtCamera _canvasName;
    ...
    void SetScene()
    {
        ...
        _progress = collider.AddComponent<PlayerProgress>();
    ...
    void SetInitialDataClientRpc(ulong id, string username, int color)
    {
        ...
        {
            _movement.GetComponentInChildren<Text>().text = username;
            _movement.model.GetComponent<MeshRenderer>().materials[1].color = _mainMenu.colors[color];
        }
    }
    ...
    void UpdateControlClientRpc(ulong id, int addScore, bool finished, string type)
    {
        if (finished && (NetworkObjectId-1) == id && _progress)
        {
            ...
            _gameControl.Ghost(_movement.model.GetComponent<MeshRenderer>());
        }
